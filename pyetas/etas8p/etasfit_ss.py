...
def cloglkhd(tht, rdata, verbose, model_module):
    ...
    # check if fault mode is on
    fault_mode = not np.all(fault == None)
    if np.sum(fault != None) > 1:
        raise Exception("Important: code for faults cannot handle the case where more than one fault geometry is present!")
    if fault_mode:
        fault_ind = np.where(fault != None)[0][0]
    ...
    for j in range(0, N):
        ...
        if not fault_mode:
            ...
            # create fictitious x and y vectors
            x2 = x.copy()
            y2 = y.copy()
            # if the current jth earthquake hasn't got a fault geometry
            if fault[j] is None:
                if fault_ind < j: # only if event j is after the ind of the fault event
                    indmin = np.argmin(dist(x2[j], y2[j],
                                            fault[fault_ind]['xx'],
                                            fault[fault_ind]['yy']))
                    x2[fault_ind] = fault[fault_ind]['xx'][indmin]
                    y2[fault_ind] = fault[fault_ind]['yy'][indmin]
            else:
                for jj in range(0, j):
                    mindist = np.min(dist(x2[jj], y2[jj], fault[j]['xx'], fault[j]['yy']))
                    # in this way the distance should be fictitiously equal to mindist
                    x2[jj] = x2[j] + mindist
                    y2[jj] = y2[j]
            
            if flag[j] == 1:
                if "lambdaf6" in model_module.__name__:
                    s = model_module.clambdaj(tht, j, t, x2, y2, m, bk, ta)
                else:
                    s = model_module.clambdaj(tht, j, t, x2, y2, m, bk)
            ...
            if "lambdaf6" in model_module.__name__:
                fv2 += model_module.cintegj(tht, j, t, x2, y2, m, npoly, px, py, tstart2, tlength, ta)
            else:
                fv2 += model_module.cintegj(tht, j, t, x2, y2, m, npoly, px, py, tstart2, tlength)
    ...
    for j in range(0, N):
        ...
        if fault[j] is None:
            ...
            if flag[j] == 1:
                ...
                if "lambdaf6" in model_module.__name__:
                    fv1temp, g1temp = model_module.clambdajGr(tht, j, t, x2, y2, m, bk, fv1temp, g1temp, ta)
                else:
                    fv1temp, g1temp = model_module.clambdajGr(tht, j, t, x2, y2, m, bk, fv1temp, g1temp)
            ...
            if "lambdaf6" in model_module.__name__:
                fv2temp, g2temp = model_module.cintegjGr(tht, j, t, x2, y2, m, npoly, px, py, tstart2, tlength, fv2temp, g2temp, ta)
            else:
                fv2temp, g2temp = model_module.cintegjGr(tht, j, t, x2, y2, m, npoly, px, py, tstart2, tlength, fv2temp, g2temp)
            fv2 += fv2temp
            for i in range(0, len(tht)):
    	        df2[i] += g2temp[i]
