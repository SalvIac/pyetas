...
'''
implementation of model 6 according to Schoenberg et al. (2013)
only the integral part (cintegj) is affected (the rest is like lambda6f)
better explained by Lippiello et al. (2014)
'''

#%% ***************************************************************************


def clambdaj(theta, j, t, x, y, m, bk, ta, faults=None, mmin=None):
...
def cintegj(theta, j, t, x, y, m, npoly, px, py, tstart2, tlength, ta,
            faults=None, mmin=None, voronoi=None):
    # extract model parameters
    A = theta[1]**2
    c = theta[2]**2
    alpha = theta[3]**2
...
def cintegjGr(theta, j, t, x, y, m, npoly, px, py, tstart2, tlength, fv, dfv,
              ta, faults=None, mmin=None, voronoi=None):
    raise Exception("not sure how to implement this with Schoenberg et al. 2013")
    # if (m[j]+3.5)<=5.49:
